...
# https://stackoverflow.com/a/28131868
random.randbytes = lambda size: bytes(random.getrandbits(8) for _ in range(size))
...
# print(len(K1),len(K2),len(K3),len(K4),len(K5),len(K6))
...
def gen_keys():
	...
	for i in range(MAGIC-1):
		...
		keys.append(key)
		k = xor(k, xor(key,flag)) # k ^ key[i] ^ flag
	keys.append(xor(k,flag)) # key[end] = k ^ flag = all keys ^ flag
	# this means that x ^ all keys = x ^ rest keys ^ rest keys ^ flag = x ^ flag
	return keys
...
def main():
	...
	# given [[0xff; MAGIC] | [0x00; MAGIC]; MAGIC]
	iv_a, iv_b = gen_iv()
	# random [[u8; MAGIC]; MAGIC], last key is all keys and flag xor'd together
	keys = gen_keys()
	# input [[u8; MAGIC]; MAGIC]
	inp = my_input()
	
	output =  b"\x00"*MAGIC			# could be seen as xor of 0 keys
	for i in range(MAGIC):
		# foo(inp[i], iv_b[i], K5, K6)
		# input . given . K5 . 56 -> out 1 (we have complete control over this)
		# random key . out 1 . K3 . K4 -> out 2 (ideally, key itself)
		# output . out 2 . K1 . K2 -> out 3 (ideally, output ^ key)
		# output must be xor of all keys to i

		# If output was xor of keys 0 to i - 1,
		# and we want output to be xor of keys 0 to i,
		# operation must be equiv to output ^= keys[i]
		...
		if not guardian(output, i, keys):
			...
			exit(0)
	# at the end, output is xor of all keys = flag
