...
.css({ position: 'absolute', top: 0, width: 'calc(100% - 40px)', /*height: 'calc(100vh - 40px)',*/ margin: '20px' })
...
const { duration } = await fetch('https://www.deltamath.com/new3/scripts/get_problems_by_assignment.php', {
method: 'POST',
body: JSON.stringify({ last_edit: Date.now() / 1000 | 0, teacher_id, sk }),
headers: {
...
const chart = new Chart(ctx, {
    ...
    data: {datasets: [{data, label: 'students',					backgroundColor: 'rgba(233, 30, 99, .61)',
					borderColor: 'rgba(233, 30, 99, 1)',fill:true}]},
...
output.append('900 just means they took at least 15 minutes which is why theres so many')
return {labels, data}
})()
